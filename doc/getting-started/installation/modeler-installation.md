...
In addition, Antares Modeler requires a `parameters.yml` file inside the study folder to run a GEMS study ; the process for creating this file is detailed in [this section](../../user-guide/file-structure/solver-optimization.md). Unlike the [GemsPy interpreter](gemspy-installation.md), Modeler depends on this configuration file.
