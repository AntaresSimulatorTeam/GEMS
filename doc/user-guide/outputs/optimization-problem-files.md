...
The MPS file follows the naming convention `{year}-{block}.mps`, where `year` is the Monte Carlo year number and `block` is the optimization block number (e.g., week or day depending on the solver configuration). For example, `1-1.mps` corresponds to year 1, block 1 and is automatically created in the output folder.
