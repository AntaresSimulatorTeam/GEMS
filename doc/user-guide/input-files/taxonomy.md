...
???+ note "Link to `catalog.yml`"
    The taxonomy is used by `catalog.yml` in [GEMS-ViewsBuilder](https://github.com/AntaresSimulatorTeam/GEMS-ViewsBuilder) to define **metrics** for each `taxonomy-category`. See [Business Views](../outputs/view.md) for more details.
