...
The **system file** defines the concrete energy system to be simulated. It instantiates [components](../../overview/glossary.md#models-and-components) from [models](../../overview/glossary.md#models-and-components) provided by the [libraries](library.md), assigns parameter values, and specifies how components are connected to each other. In practice, the system file answers questions such as which components are included, how they are linked, what parameter values they use (fixed, time-dependent, or scenario-dependent), and which model libraries are required. It acts as the central configuration that brings together models, data, and connections for a simulation run.
...
| Element | Type | Description |
|------|------|--------------------------|
| `id` | String | The property key name, matching one declared in the [model's properties](library.md#properties). |
...
Port `connections` determine how linear expressions are exchanged between components. If a model defines a [port](library.md#ports) only in the ports section, it acts as a **receiver** for that port and collects linear expressions emitted by connected components. A typical example is a **bus** model, which receives flow expressions from connected components (generators, load etc.).

If a model additionally defines [**port-field-definitions**](library.md#port-field-definition), it acts as an **emitter** for that port. In this case, the model exposes linear expressions through the port, allowing connected receiver component to consume them. A common example is a generator model, which emits it's `generation` variable to connected bus component.
