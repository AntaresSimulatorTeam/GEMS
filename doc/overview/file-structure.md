...
| **[Views Configurations](../user-guide/input-files/view-configuration.md)**| <span style="display:inline-block; width:12px; height:12px; background-color:#8B5FB5; border-radius:50%; margin-right:5px;"></span>Business Intelligence| YAML (`view-config.yml`) | **Views Logic:** Define the interconnection between groups mentionned inside `taxonomy.yml` and the metrics defined by `catalog.yml`.|
| **[Metrics Catalog](../user-guide/input-files/catalog.md)**| <span style="display:inline-block; width:12px; height:12px; background-color:#8B5FB5; border-radius:50%; margin-right:5px;"></span>Business Intelligence| YAML (`catalog.yml`) | **Metrics Logic:** Configure the metrics settings for views. It uses by `view-config.yml` |
