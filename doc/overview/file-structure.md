...
| **[Metrics Catalog](../user-guide/file-structure/catalog.md)**| <span style="display:inline-block; width:12px; height:12px; background-color:#8B5FB5; border-radius:50%; margin-right:5px;"></span>Business Intelligence| YAML (`catalog.yml`) | **Metrics Logic:** Defines a set of BusinessMetrics with their aggregation rules (terms, terms-operator, time-operator). Used by `view-config.yml`.|
